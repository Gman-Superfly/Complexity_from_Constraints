...
- **Noisy Channel Coding / Redundancy**: In `Hallucinations_Noisy_Channels`, we framed hallucinations as "uncorrected errors" in a noisy channel. The semantic redundancy threshold $\rho$ acts as the "parity bit" that allows future context to redeem earlier distortions. This connects learning dynamics to Shannon's channel capacity, where redundancy is not waste but the price of reliability.
